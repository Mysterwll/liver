...
def prepare_to_train(model_index, seed, device):
    ...
    try:
        model_settings = models[model_index]
    except KeyError:
    ...
    target_dir = target_dir.joinpath(model_settings['Name'] + current_time)
    target_dir.mkdir(exist_ok=True)
    ...
    observer = Runtime_Observer(log_dir=log_dir, device=device, name=model_settings['Name'], seed=seed)
    observer.log(f'[DEBUG]Observer init successfully, program start @{current_time}\n')
    '''
    Model load
    '''
    _model = models[model_index]['Model']
    model = _model()
    observer.log(f'Use model : {str(model_settings)}\n')
    ...
    '''
    optimizer = model_settings['Optimizer'](model.parameters(), model_settings['Lr'])
    criterion = model_settings['Loss']()

    print("prepare completed! launch training!\U0001F680")

    # launch
    _run = model_settings['Run']
    _run(observer, model_settings['Epoch'], trainDataLoader, testDataLoader, model, device, optimizer, criterion)
