...
def train_epoch(Radio_encoder, Image_encoder, trainDataLoader, optimizer, criterion, device):
    train_loss = AverageMeter()
    Radio_encoder.train()
    Image_encoder.train()
    train_bar = tqdm(trainDataLoader, leave=True, file=sys.stdout)
    for i, (_, _, _, radio, img, _ ) in enumerate(train_bar):
...
def test_epoch(Radio_encoder, Image_encoder, valDataLoader, criterion, device):
    test_loss = AverageMeter()
    Radio_encoder.eval()
    Image_encoder.eval()
    val_bar = tqdm(valDataLoader, leave=True, file=sys.stdout)
    for i, ( _, _, _, radio, img, _ ) in enumerate(val_bar):
        # print("radio.shpae = " + str(radio.shape))
        # print("img.shpae = " + str(img.shape))
        radio = radio.to(device)
        img = img.to(device)
        ...
        test_loss.update(loss.item(), radio.shape[0])
...
def train(batch_size, epochs, optimize_selection, learning_rate, name, seed, device):
    ...
    Model load
    '''
    Radio_encoder = Radiomic_encoder(num_features=1781).to(device)
    ...
    for epoch in range(epochs):
        ...
        train_loss = train_epoch(Radio_encoder, Image_encoder, trainDataLoader, optimizer, criterion, device)

        val_loss = test_epoch(Radio_encoder, Image_encoder, valDataLoader, criterion, device)
    ...
    observer.log( "---experiment ended---\n" \
           + "Best Epoch %d:\n" % (best_epoch+1) \
           + "Best Val Loss : %4.2f" % (best_loss) )
